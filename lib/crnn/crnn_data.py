...
class InputGenerator(object):
    ...
    def __init__(self, gt_util_real, batch_size, alphabet, input_size=(255,32),
                grayscale=True, max_string_len=30,shuffle=True,gt_util_synth=None):
        
        self.__dict__.update(locals())
    
    def idx(self,synth=False):
        if synth == False: 
            if self.i_real == self.gt_util_real.num_samples : 
                self.idxs_real = np.arange(self.gt_util_real.num_samples)
                if self.shuffle : 
                    np.random.seed(1337)
                    np.random.shuffle(self.idxs_real)
                self.i_real = 0
                print('New epoch (real)')
            idx = self.idxs_real[self.i_real]
            self.i_real += 1 
            return idx
                
        if synth : 
            if self.i_synth == self.gt_util_synth.num_samples : 
                self.idxs_synth = np.arange(self.gt_util_synth.num_samples)
                if self.shuffle : 
                    np.random.seed(1337)
                    np.random.shuffle(self.idxs_synth)
                self.i_synth = 0
                print('New epoch (real)')
            idx = self.idxs_synth[self.i_synth]
            self.i_synth += 1 
            return idx
    ...
    def generate(self, train=True):
        gt_util_real = self.gt_util_real
        gt_util_synth = self.gt_util_synth
        ...
        # NOTE : mix synthetic dataset 
        mix_mode = (gt_util_synth != None)
        self.i_real = self.gt_util_real.num_samples

        if mix_mode  : 
            self.i_synth = self.gt_util_synth.num_samples

        while True:
            while len(targets) < batch_size:
                if mix_mode and np.random.binomial(1,0.5) == 1  : 
                    gt_util = gt_util_synth
                    idx = self.idx(synth=True)
                else : 
                    gt_util = gt_util_real
                    idx = self.idx(synth=False)
                ...
                mask_text = np.array([w != None for w in texts])
                mask = mask_text
