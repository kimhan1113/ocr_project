...
from ..ssd.ssd_model import ssd512_body
from ..ssd.ssd_model_dense import dsod512_body
...
def TBPP512_dense(input_shape=(512, 512, 3), softmax=True):
    ...
    # DSOD body
    x = input_tensor = Input(shape=input_shape)
    # x.shape = batch x 512 x 512 x 3 
    #x = input_tensor = Input(batch_shape=(1,None,None,3))
...
def get_network(name,input_shape=(512,512,3),softmax=False):
    if name == 'dense':
        freeze = []
        return TBPP512_dense(input_shape=input_shape,softmax=softmax),freeze
    elif name == 'vgg16':
        ...
        return TBPP512(input_shape=input_shape,softmax=softmax),freeze
