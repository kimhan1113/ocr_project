...
from ..ssd.ssd_metric import fscore
...
def filter(detection_polys,body_polys) : 
    '''
    ...
    dt_poly = []
    for i in range(len(detection_polys)): # samples

        if is_inside(detection_polys[i],body_polys[0]) : 
            dt_poly.append(detection_polys[i])
...
def evaluate_polygonal_results(ground_truth, detection_results,body=None, iou_thresh=0.75):
    ...
    for i in range(len(gt)): # samples
        ...
        if body != None : 
            body_polys = [np.reshape(body[i][j,:], (-1, 2)) for j in range(len(body[i]))]
            dt_polys = filter(dt_polys,body_polys)
        ...
        for k in range(len(dt_polys)): # dt
            poly1 = dt_polys[k]
            gt_iou = []
